...
// 在一个thunk中可以完成一系列连续的action操作
// 并且可以处理异步逻辑
// 业务逻辑可以从ui层面挪到这里，代码分层会更加清晰
export const giveMeDataActionCreator = (): ThunkAction<void, RootState, unknown, RecommendProductAction> => async (dispath, getState) => {
    dispath(fetchRecommendProductStartActionCreator);
    try {
      const { data } = await axios.get(
        "http://123.56.149.216:8080/api/productCollections"
      );
      dispath(fetchRecommendProductSuccessActionCreator(data));
    } catch (error) {
        dispath(fetchRecommendProductFailActionCreator(error instanceof Error ? error.message : "error"));
